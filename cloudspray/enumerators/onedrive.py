...
How it works:
    ...
    For example, ``jane.doe@example.com`` becomes ``jane_doe_example_com``.
...
class OneDriveEnumerator:
    ...
    replaced by underscores (e.g. ``john_smith_example_com``).
    ...
    Usage example::
        ...
        enumerator = OneDriveEnumerator("example.com", db, reporter)
        valid = enumerator.enumerate(["john.smith", "jane.doe@example.com"])

    Args passed to ``__init__``:
        domain: Target tenant domain (e.g. ``example.com``). The tenant
    ...
    def __init__(
        ...
        self._domain = domain
        # Extract the tenant slug (e.g. "example" from "example.com") used to
        # build the SharePoint "-my" subdomain.
        self._tenant = self._derive_tenant_slug(domain)
        self._db = db
    ...
    @staticmethod
    def _derive_tenant_slug(domain: str) -> str:
        """Extract tenant slug from domain for SharePoint personal site URLs.

        For simple domains (example.com), returns 'example'.
        For subdomains (corp.example.com), returns 'example' (second-level domain).
        """
    ...
    def _format_username(self, email: str) -> str:
        ...
        Args:
            email: Full email address (e.g. ``jane.doe@example.com``).

        Returns:
            URL path segment (e.g. ``jane_doe_example_com``).
        """
