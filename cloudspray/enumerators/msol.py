...
class MSOLEnumerator:
    ...
    Usage example::
        ...
        enumerator = MSOLEnumerator("example.com", db, reporter)
        valid = enumerator.enumerate(["john.smith", "jane.doe@example.com"])

    Args passed to ``__init__``:
        domain: Target tenant domain (e.g. ``example.com``).
    ...
    def check_user_exists(self, email: str) -> bool | None:
        """Check if a single user exists. Public API for format discovery.

        Returns True if user exists, False if not, None if indeterminate.
        """
