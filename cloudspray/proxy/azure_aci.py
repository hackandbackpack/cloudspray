...
CONTAINER_IMAGE = "tinyproxy/tinyproxy:1.11"  # Lightweight HTTP/HTTPS proxy (pinned version)
...
class AzureACIProvider(ProxyProvider):
    ...
    def _get_public_ip() -> str:
        """Get the operator's public IP for proxy allowlisting."""
    ...
    def setup(self, target_url: str) -> None:
        ...
        # Get operator IP for proxy allowlisting
        operator_ip = self._get_public_ip()
        if not operator_ip:
            logger.warning(
                "Could not determine public IP for proxy allowlist. "
                "Containers will be open."
            )

        # Build tinyproxy config with optional IP restriction
        allow_line = f"Allow {operator_ip}\\n" if operator_ip else ""
        startup_cmd = [
            "/bin/sh",
            "-c",
            f"printf 'Port {PROXY_PORT}\\nTimeout 600\\nMaxClients 100\\n"
            + allow_line
    ...
    def get_proxy_url(self) -> str:
        ...
        Returns:
            Proxy URL like "http://198.51.100.1:8888".
