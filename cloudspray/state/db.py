...
class StateDB:
    ...
    def __init__(self, db_path: str | Path):
        ...
        # Restrict DB file to owner-only read/write (0600).
        # Silently ignored on Windows which lacks Unix permission bits.
        try:
            os.chmod(db_path_str, stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    ...
    def filter_unattempted_pairs(
        self, pairs: list[tuple[str, str]]
    ) -> list[tuple[str, str]]:
        """Return only pairs not already attempted. Uses SQL for efficiency."""
        if not pairs:
            return []
        ...
        with self._transaction() as cur:
            ...
            chunk_size = 450  # SQLite default SQLITE_MAX_VARIABLE_NUMBER is 999
            for i in range(0, len(pairs), chunk_size):
                chunk = pairs[i : i + chunk_size]
                placeholders = " OR ".join(
                    ["(username = ? AND password = ?)"] * len(chunk)
                )
                flat_params = [v for pair in chunk for v in pair]
                cur.execute(
                    f"SELECT username, password FROM spray_attempts WHERE {placeholders}",
                    flat_params,
                )
                attempted.update((row[0], row[1]) for row in cur.fetchall())

        return [p for p in pairs if p not in attempted]
